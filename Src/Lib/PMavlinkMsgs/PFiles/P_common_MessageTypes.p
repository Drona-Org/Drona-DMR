...
_fix_type : int,  /*0-1: no fix, 2: 2D fix, 3: 3D fix, 4: DGPS, 5: RTK. Some applications will not use the value of this field unless it is at least two, so always correctly fill in the fix.*/
...
_time_utc : int,  /*Timestamp (microseconds since UNIX epoch) in UTC. 0 for unknown. Commonly filled by the precision time source of a GPS receiver.*/
_time_boot_ms : int,  /*Timestamp (milliseconds since system boot)*/
_lat : int,  /*Latitude, expressed as degrees * 1E7*/
_lon : int,  /*Longitude, expressed as degrees * 1E7*/
...
_time_utc : int,  /*Timestamp (microseconds since UNIX epoch) in UTC. 0 for unknown. Commonly filled by the precision time source of a GPS receiver.*/
_time_boot_ms : int,  /*Timestamp (milliseconds since system boot). 0 for system without monotonic timestamp*/
_x : float32,  /*X Position*/
_y : float32,  /*Y Position*/
_z : float32,  /*Z Position*/
...
_vel : int,  /*GPS ground speed (m/s * 100). If unknown, set to: UINT16_MAX*/
_cog : int,  /*Course over ground (NOT heading, but direction of movement) in degrees * 100, 0.0..359.99 degrees. If unknown, set to: UINT16_MAX*/
_fix_type : int,  /*0-1: no fix, 2: 2D fix, 3: 3D fix, 4: DGPS fix, 5: RTK Fix. Some applications will not use the value of this field unless it is at least two, so always correctly fill in the fix.*/
_satellites_visible : int,  /*Number of satellites visible. If unknown, set to 255*/
...
_time_usec : int,  /*Timestamp (milliseconds since system boot)*/
