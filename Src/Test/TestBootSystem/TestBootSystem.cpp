...
static void LogHandler(PRT_STEP step, PRT_MACHINEINST *sender, PRT_MACHINEINST *receiver, PRT_VALUE* event, PRT_VALUE* payload)
{
	...
	PRT_STATEDECL* state = PrtGetCurrentStateDecl(c);
	if (state == NULL) {
		stateName = L"model";
	}
	else {
		stateName = ConvertToUnicode((const char*)state->name);
	}
	std::wstring eventName;
	std::wstring stateId = machineName + L"." + stateName;
	std::wstring stateLabel = stateId;
	...
	if (sender != NULL && event != NULL)
	{
		...
		senderMachineName = GetMachineName(s);
		PRT_STATEDECL* state = PrtGetCurrentStateDecl(s);
		senderStateName = (state == NULL) ? ConvertToUnicode("model") : ConvertToUnicode((const char*)state->name);
		senderStateId = senderMachineName + L"." + senderStateName;
		senderStateLabel = senderStateId;
	}
...
void RunMachine() {
	...
	PrtMkMachine(MAIN_APPLICATION_PROCESS, P_MACHINE_MissionPlannerMachine, payload);
